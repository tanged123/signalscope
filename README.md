...
[![SignalScope interactive demo](https://tanged123.github.io/signalscope/demo.gif?v=1.1.9)](https://tanged123.github.io/signalscope/demo.html)
...
from a snapshot data slot. Live time panels use one uniform, budgeted
bins-per-physical-pixel density across the active layout. Prepared CPU and GPU
overview/detail banks remain resident when budgets allow, inactive banks are
evicted by managed bytes, and zoom refines from pixel-dense envelopes to exact
level-zero samples. The UI reports managed presentation pressure; WebGPU does
not expose total or free VRAM. `HttpPlane` and `BakedPlane` use the same
adaptive presentation contract, while explicitly reduced-fidelity snapshots
and HTML or CSV exports retain their independent fidelity controls; the
renderer has no host-specific branch.
