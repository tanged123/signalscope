...
./scripts/build.sh appimage            Linux AppImage build
...
The Nix flake supplies the normal pinned toolchain. Windows packaging is the
...
- One TypeScript/canvas presentation plane runs in both the Electron renderer and
  the self-contained HTML snapshot.
- The native host uses `NativePlane`; snapshots use `BakedPlane`; both implement
...
- Rust owns ingest, storage, pyramids, compute, persistence, and IPC-facing
  data. Electron stays thin: windows, dialogs, protocol wiring, and lifecycle.
...
The dependency direction is inward: ingest/pyramid/compute use store; the
Rust host uses protocol and core crates; frontend uses generated protocol types. An
