...
Per-panel Plot settings groups appearance and readout controls, while X/Y/C
bindings remain visible. The global legend menu and inactive Follow slot are
...
Native disclosures use `ui/disclosure.ts` for Escape/outside dismissal and
listener cleanup; panel disposal and page teardown release these listeners.
...
Performance details report the existing CPU chart-update duration and density
...
Always-visible appearance controls are quicker to scan but crowd out bindings;
the disclosure keeps the same actions keyboard accessible on demand.
...
per-plot menus, and compact status layout. Existing snapshot checks validate the
