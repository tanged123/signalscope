...
3. **Dock footer (from §10):** the rail closes with the aggregate `N sources · M signals` + right-aligned `X pts`, and a second line `+ source` + right `CSV · MCAP` (both act: `+ source` opens the load dialog, format labels are inert). This replaces whatever the spinner-stack deletion (P6 Task 3) left behind; sits below the source rows.
