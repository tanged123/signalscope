...
- [ ] **Step 1: Write the ADR.** Record the decision, not the deliberation; match the tone/length of ADR 0026 or 0033. Required content:
  ...
  - The interaction budgets: a core tile-refresh p95 floor of 20 ms and a browser frame p95 floor of 33 ms (the 30 fps frame budget), enforced independently — tile refresh overlaps frame production, and per-stage p95 values do not compose into an end-to-end p95, so no summed end-to-end budget is claimed; stalls over 250 ms fail.
