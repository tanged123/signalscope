...
[ADR 0063](adr/0063-logarithmic-axis-scales.md). Limits also offers independent
horizontal and vertical axis flips, including saved/offline state and matching
navigation ([ADR 0064](adr/0064-axis-direction.md)). Broad validation runs in CI.
...
- Chrome keeps dataset totals and chart metrics in the status bar. Each plot
  ...
  memory/density estimates without adding profiling work. Unsaved file changes
  are labeled in the status bar. UI labels, muted text, menus, and the signal
  tree share the selected UI font and scalable size tokens (ADR 0058).
