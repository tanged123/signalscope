...
Transforms: docked formula bar — `derived/name = expr`, quoted full-path references (`'source/group/signal'`), `gradient`/`cumtrapz`/`movmean`, MATLAB operators and scalar functions; ↵ create, ↑/↓ history, ctrl+space complete, esc collapse. Drag tree signals into the bar to insert references. Quick transforms are duplicated in the legend inspector.
